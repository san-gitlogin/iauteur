...
Two different ways to answer questions about a test:

  - StubAIProvider: no network call at all, a small set of plain rules
    based on the test's status, plus a canned reply for general chat.
    Always available, always instant, and it means this whole chapter
    is testable even with zero setup.

  - AzureOpenAIProvider: real calls to Azure OpenAI's chat completions
...
Each provider offers two methods: `analyze()` (one-shot, used by
Chapter 8's endpoint) and `stream_chat()` (a back-and-forth
conversation, yielding pieces of the answer as they become available -
used by Chapter 9's chat endpoint).
...
class AIProvider(ABC):
    ...
    def stream_chat(self, messages: list[dict]) -> Iterator[str]:
        """Given a full conversation (system + history + latest user message), yield the reply piece by piece."""
...
class StubAIProvider(AIProvider):
    ...
    def stream_chat(self, messages: list[dict]) -> Iterator[str]:
        ...
        reply = (
            "I'm running without a real AI account right now, so I can only give a canned "
            "reply. You asked: " + last_user_message[:200] + " -- once real Azure OpenAI "
            "credentials are set, I'll actually read the conversation and answer properly."
        )
        for word in reply.split(" "):
            yield word + " "
            time.sleep(0.03)  # tiny delay so the frontend visibly streams, even without a real AI
