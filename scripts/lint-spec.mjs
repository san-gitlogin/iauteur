...
const subTypeOf = (s) => {
  ...
  // ASTRA_STAGE is the same shape of thing and is counted the same way: one registered
  // type dispatching a different DEPICTION per `kind` — a pair of notebooks, a balance,
  // a ladder, a stack of paper. Counting the type alone would read sixteen distinct
  ...
  // MEM_STAGE, the same again: twenty-five different pictures behind one type.
  if (s.type === 'MEM_STAGE') return `MEM_STAGE:${s.data?.memStage?.kind ?? '?'}`;
...
for (const s of spec.scenes ?? []) {
  ...
  if (d.liveCode) {
    const lc = d.liveCode;
    const ls = lc.lines ?? [];
    if (!ls.length) E(`${id}: LIVE_CODE needs at least one line`);
    if (ls.length > 14) E(`${id}: LIVE_CODE max 14 lines (got ${ls.length}) — split the file across two beats`);
    if ((lc.before ?? []).length > 8) E(`${id}: liveCode max 8 pre-existing lines`);
    if ((lc.output ?? []).length > 6) E(`${id}: liveCode max 6 output lines`);
    if (len(lc.headline) > 48) E(`${id}: liveCode headline > 48 chars`);
    if (len(lc.filename) > 26) E(`${id}: liveCode filename > 26 chars`);
    if (len(lc.runCmd) > 44) E(`${id}: liveCode runCmd > 44 chars`);
    ...
    for (const o of lc.output ?? []) {
      if (String(o.text ?? '').length > 64) E(`${id}: liveCode output line > 64 chars`);
    }
    // THE SYNC IS THE COMPONENT. A line without its own word types across somebody else's
    // interval, which is precisely the drift this type exists to remove.
    const noAnchor = ls.filter((l) => typeof l.atWord !== 'number');
    if (noAnchor.length)
      E(`${id}: LIVE_CODE has ${noAnchor.length} line(s) with no atWord — a line types from ITS word to the next line's, so one missing anchor desynchronises everything after it`);
    for (let i = 1; i < ls.length; i++) {
      if (typeof ls[i].atWord === 'number' && typeof ls[i - 1].atWord === 'number' &&
          ls[i].atWord < ls[i - 1].atWord)
        E(`${id}: liveCode line ${i + 1} is anchored before line ${i} — a file is written top to bottom`);
    }
    if (typeof lc.runAtWord === 'number' && ls.length) {
      const last = ls[ls.length - 1].atWord;
      if (typeof last === 'number' && lc.runAtWord < last)
        E(`${id}: liveCode runAtWord (${lc.runAtWord}) precedes the last line (${last}) — the command cannot run a file that is not written yet`);
    }
    if (typeof lc.outAtWord === 'number' && typeof lc.runAtWord === 'number' &&
        lc.outAtWord < lc.runAtWord)
      E(`${id}: liveCode outAtWord precedes runAtWord — output before the command`);
    checkColor(id, 'liveCode.color', lc.color);
  }
  ...
  if (d.memStage) {
    const u = d.memStage;
    const KINDS = ['host-guest', 'page-wall', 'evict-paths', 'oom-kill', 'pool-borrow', 'gf-xor', 'sparse-encode', 'dense-fallback', 'chart-literals', 'sim-constants', 'declared-vs-real', 'test-rig', 'codec-bars', 'page-mix', 'sparsity-ruler', 'slab-buckets', 'decode-race', 'no-cap', 'abba-lock', 'fault-path', 'xbzrle-delta', 'memory-contract', 'fast-path', 'roadmap', 'verdict'];
    if (!u.kind) E(`${id}: MEM_STAGE needs a kind`);
    else if (!KINDS.includes(u.kind))
      E(`${id}: MEM_STAGE unknown kind "${u.kind}" — must be one of ${KINDS.join(', ')}. It would render as UNKNOWN DEPICTION KIND.`);
    // The headline shares APPLE_STAGE's Headline. The premise is capped at 120 so it stays
    // ONE line at 16:9 and the picture keeps the pane height it was laid out for.
    if (len(u.headline) > 44) E(`${id}: MEM_STAGE headline > 44 chars`);
    if (len(u.premise) > 120) E(`${id}: MEM_STAGE premise > 120 chars (LAW 0l: readable at a glance)`);
    if (len(u.caption) > 30) E(`${id}: MEM_STAGE caption > 30 chars`);
    if (len(u.token) > 40) E(`${id}: MEM_STAGE token > 40 chars`);
    checkColor(id, 'memStage.color', u.color);
    const cl = u.cells ?? [];
    // Per-kind CEILINGS: each picture's real capacity, not a round number (LAW 0n).
    const MAXI = {'host-guest': 6, 'page-wall': 6, 'evict-paths': 9, 'oom-kill': 7, 'pool-borrow': 5,
                  'gf-xor': 5, 'sparse-encode': 5, 'dense-fallback': 4, 'chart-literals': 4,
                  'sim-constants': 7, 'declared-vs-real': 3, 'test-rig': 7, 'codec-bars': 12,
                  'page-mix': 10, 'sparsity-ruler': 4, 'slab-buckets': 4, 'decode-race': 5,
                  'no-cap': 3, 'abba-lock': 7, 'fault-path': 11, 'xbzrle-delta': 4,
                  'memory-contract': 4, 'fast-path': 4, 'roadmap': 6, 'verdict': 3};
    const NEED = {'codec-bars': 2, 'page-mix': 2, 'decode-race': 2, 'roadmap': 2, 'fault-path': 3,
                  'abba-lock': 4, 'chart-literals': 2, 'declared-vs-real': 2, 'test-rig': 3};
    if (u.kind && MAXI[u.kind] && cl.length > MAXI[u.kind])
      E(`${id}: MEM_STAGE kind "${u.kind}" draws at most ${MAXI[u.kind]} cell(s), got ${cl.length}`);
    if (u.kind && NEED[u.kind] && cl.length < NEED[u.kind])
      E(`${id}: MEM_STAGE kind "${u.kind}" needs >= ${NEED[u.kind]} cell(s), got ${cl.length}`);
    for (const x of cl) {
      if (len(x.label) > 44) E(`${id}: MEM_STAGE cell label "${x.label}" > 44 chars`);
      if (len(x.sub) > 60) E(`${id}: MEM_STAGE cell sub > 60 chars`);
      if (len(x.text) > 24) E(`${id}: MEM_STAGE cell text "${x.text}" > 24 chars`);
      checkColor(id, 'memStage.cells.color', x.color);
    }
