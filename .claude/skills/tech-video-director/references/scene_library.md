...
| MEM_STAGE | a memory-systems beat (paging, swap, compression, allocators, a benchmark's provenance) drawn as the OBJECT it is about, one purpose-built picture per `kind`: host-guest, page-wall, evict-paths, oom-kill, pool-borrow, gf-xor, sparse-encode, dense-fallback, chart-literals, sim-constants, declared-vs-real, test-rig, codec-bars, page-mix, sparsity-ruler, slab-buckets, decode-race, no-cap, abba-lock, fault-path, xbzrle-delta, memory-contract, fast-path, roadmap, verdict | headline≤44, kind(req), cells(≤12, per-kind cap), vars, caption≤30, premise≤120, token≤40, color, atWord |
...
| MODEL_SHRUG | A beat whose whole point is that something is MISSING from a list it should have been on: a value stated and then not flagged, a check that never ran, a case a rule does not cover. Not for comparing two lists (SPEC_COMPARE) and not for a claim that is wrong (CLAIM_CHECK) - this is for a claim that is absent. | headline≤48, needle≤22, saidLabel≤22, missedLabel≤22, said, missed, verdict≤40, verdictAtWord, source≤64, atWord |
| CLAIM_CHECK | A specific, checkable assertion is on screen and the beat is about whether it holds - a model's confident finding, a benchmark headline, a claim in a changelog. Use it when the check is a COUNT you can draw. Not for a value that is merely absent from a list (MODEL_SHRUG) and not for two products' spec sheets (SPEC_COMPARE). | headline≤48, claims, subject≤14, tallyLabel≤28, tally, hitLabel≤10, verdict≤44, verdictAtWord, source≤64, atWord |
| COLUMN_SPLIT | A beat about information lost by SUMMARISING per field: a column profile that cannot answer a cross-column question, an aggregate that cannot be un-aggregated, a log stripped to counts. The lesson is the missing JOIN, and this component is the only one that draws a link that is not there. Not for a hierarchy (DIAGRAM tree) and not for a query over rows (DATABASE_TABLE). | headline≤48, tableName≤20, columns, rows, splitAtWord, left, right, question≤46, askAtWord, gapNote≤26, source≤64, atWord |
| LIVE_CODE | ANY beat where the viewer is meant to WRITE the code along with you - a from-scratch tutorial, a file being created, a fix being typed into an existing file. CODE_RUN reveals a block that is already on screen and CODE_WINDOW types at a fixed speed from one start; this one is the only shape where the typing itself is anchored per line, which is what makes a voiceover and a keyboard land together. Not for a finished listing nobody is asked to write (CODE_WINDOW). | headline≤48, filename≤26, language, newFile, before, lines, runCmd≤44, runAtWord, output, outAtWord, caption≤40, source≤64, atWord, color |
