---
name: design-botanical-organic-serif
description: Use when the user wants a video, theme, or component set in the "Botanical / Organic Serif" design style, asks for design variety suggestions for their Remotion video studio, or references this aesthetic. Provides the complete Botanical / Organic Serif design language (colors, typography, principles) plus mandatory rules for adapting it into a video-safe Remotion theme per the studio's design contract.
---

# Botanical / Organic Serif — Video Design Skill

## How to use this design for VIDEO (mandatory adaptation)
This is a WEB design system. Video adaptation rules override anything below:
1. Convert tokens into a `VideoTheme` object for `src/themes.ts` (colors incl. a 6-hue semantic palette with meaning [green=works, red=broken, blue=info, purple=AI, orange=tension, yellow=cost], fonts by role [display/body/mono/accent], zones, bgStyle, softSurface, logoMono for dark styles).
2. Motion: everything frame-driven (`useCurrentFrame` + interpolate/spring, clamped). Halve all speeds and background opacities — video backgrounds must whisper.
3. Full compliance with the studio's design_contract.md: Three Guards on every bounded text, ×scale sizing, both-aspect (1920x1080 AND 1080x1920) proof stills before merge.
4. Ship with: gallery demo scene, scene_library row, text budgets, linter entries.
5. Interactive states (hover/focus) become timeline moments (word anchors); form elements are skipped.
