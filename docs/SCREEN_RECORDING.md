...
- [x] c. **Press every one of them for real.** `node scripts/probe-keys.mjs --probe`. A chord counts
      ...
      **RESULT: 103 verified, 0 failing, 9 unverified, of 112 pressable chords** (run 10,
      `briefs/vscode-shortcuts/VERIFIED.md`, generated by `scripts/keys-report.mjs`). Every one of
      the nine carries a MEASURED reason - no debug extension, no diagnostic in the file, a state
      that lives in a context key rather than the DOM, a browser that refuses synthetic fullscreen.
      None is a guess.
- [x] d. Teach the runner a general `keys` action (any chord, incl. two-key chords) with read-back.
      `verify` is REQUIRED and names the observable that must move; a step that presses without
      checking is how a shortcut silently does nothing on camera.
...
- **Give every probe its own file.** Resetting a shared buffer was tried twice and failed twice:
  the first probe cut a line out of `plain.txt` and five later probes inherited the damage - two of
  which then PASSED on the damage. A fresh copy per row makes table order irrelevant, which is the
  only property that makes a 112-row suite worth trusting. The same rule killed a second
  order-dependence: `f5` reported nothing because `ctrl+shift+d` three rows earlier had already
  opened the view it opens.
- **`navigator.clipboard.readText()` HANGS, it does not reject**, when the document is not focused.
  A `.catch()` never runs because nothing throws. It killed a full run stone dead at row 99.
- **One row that cannot answer must cost one row, not the run.** Opening the Extensions view in a
  workspace with no reachable marketplace leaves the renderer unable to answer, and seven working
  shortcuts after it timed out. Every row now has a deadline and a reload-and-retry.
- **A partial run may not write the tracked table.** A `--only=Debug` pass overwrote
  `verified.json` with three rows. A partial measurement presented as a complete one is worse than
  no measurement.
- **Chain `replace()` calls without asserting and they no-op in silence.** Two fixes in one round
  reported success and changed nothing; the failures they were meant to fix came back looking like
  VS Code bugs. Every patch asserts its anchor now.
