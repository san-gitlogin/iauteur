...
// TITLE_CARD — gradient wordmark title + pill subtitle. Calm, corporate.
export const CtTitleCard: React.FC<{scene: Scene}> = ({scene}) => {
  ...
  return (
    <AbsoluteFill style={{alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 32 * scale, padding: 80 * scale}}>
      <div style={{...fadeUp(frame, 0, fps), ...gradText(), fontFamily: t.fonts.display, fontWeight: 800, fontSize: (vertical ? 80 : 104) * scale, letterSpacing: '-0.02em', textAlign: 'center', maxWidth: '88%', lineHeight: 1.05}}>{d.title}</div>
      {d.subtitle ? (<div style={{...fadeUp(frame, 14, fps)}}><Pill>{d.subtitle}</Pill></div>) : null}
...
// STAT_PANELS — elevated cards with gradient numbers + emerald check.
export const CtStatPanels: React.FC<{scene: Scene}> = ({scene}) => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  const t = useTheme();
  const {scale, vertical} = useScale();
  ...
  return (
    <AbsoluteFill>
      {d.headline ? <CtHeadline text={d.headline} /> : null}
        ...
        {stats.map((stat, i) => {
          ...
          return (
            <div key={i} style={{...stackIn(frame, start, fps)}}>
              <Card tint={TINTS[i % TINTS.length]} style={{minWidth: (vertical ? 560 : 360) * scale, display: 'flex', flexDirection: 'column', gap: 16 * scale, alignItems: 'flex-start'}}>
                <div style={{fontFamily: t.fonts.body, fontWeight: 600, fontSize: 24 * scale, letterSpacing: '0.02em', textTransform: 'uppercase', color: t.colors.muted}}>{stat.kicker}</div>
                <div style={{...gradText(), fontFamily: t.fonts.display, fontWeight: 800, fontSize: (vertical ? 92 : 84) * scale, fontVariantNumeric: 'tabular-nums', lineHeight: 1, letterSpacing: '-0.02em'}}>{stat.value}</div>
              </Card>
            </div>
          );
        })}
...
export const CtStepFlow: React.FC<{scene: Scene}> = ({scene}) => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  const t = useTheme();
  const {scale, vertical} = useScale();
        ...
        <div style={{display: 'flex', flexDirection: vertical ? 'column' : 'row', gap: 30 * scale, alignItems: 'stretch'}}>
          {steps.map((step, i) => (
            <div key={i} style={{...springPop(frame, wordToFrame(step.atWord), fps)}}>
              <Card tint={TINTS[i % TINTS.length]} style={{width: (vertical ? 540 : 300) * scale, display: 'flex', flexDirection: 'column', gap: 18 * scale}}>
                <GradTile size={60} radius={16}>
                  <span style={{fontFamily: t.fonts.display, fontWeight: 800, fontSize: 30 * scale, color: '#fff'}}>{i + 1}</span>
                </GradTile>
...
// LIST_BUILD — elevated rows with a gradient icon tile + emerald check.
export const CtListBuild: React.FC<{scene: Scene}> = ({scene}) => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  const t = useTheme();
  const {scale, vertical} = useScale();
    ...
    <AbsoluteFill style={{alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 22 * scale, padding: 86 * scale}}>
      ...
      {items.map((item, i) => (
        <div key={i} style={{...stackIn(frame, wordToFrame(item.atWord), fps), width: vertical ? '94%' : '70%'}}>
          <Card tint={TINTS[i % TINTS.length]} style={{padding: `${20 * scale}px ${26 * scale}px`, display: 'flex', alignItems: 'center', gap: 24 * scale}}>
            ...
            <div style={{display: 'flex', flexDirection: 'column', gap: 4 * scale, flex: 1}}>
              <div style={{fontFamily: t.fonts.display, fontWeight: 700, fontSize: (vertical ? 38 : 36) * scale, color: t.colors.text, lineHeight: 1.2, letterSpacing: '-0.01em'}}>{item.text}</div>
...
// STAT_CALLOUT — giant indigo→violet gradient number over a soft orb.
export const CtStatCallout: React.FC<{scene: Scene}> = ({scene}) => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();
  const t = useTheme();
  const {scale, vertical} = useScale();
  ...
  return (
    <AbsoluteFill style={{alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 34 * scale}}>
      <div style={{...springPop(frame, start, fps), position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
        <div style={{position: 'absolute', width: 640 * scale, height: 640 * scale, borderRadius: '50%', background: 'radial-gradient(circle, rgba(99,102,241,0.20) 0%, rgba(139,92,246,0.12) 40%, transparent 68%)', filter: `blur(${34 * scale}px)`}} />
        <div style={{...gradText(), position: 'relative', fontFamily: t.fonts.display, fontWeight: 800, fontSize: (vertical ? 210 : 250) * scale, fontVariantNumeric: 'tabular-nums', lineHeight: 0.95, letterSpacing: '-0.03em'}}>
