...
// CHAPTER bound to a pack kit.
//
// It used to draw the composition itself, and that is the whole problem: all SIXTY packs call
// this factory, so every chapter card of every course ever rendered was the same slide — mono
// kicker, 290px numeral, ruled diamond, title. Owner, alongside the hook complaint: *"we need
// to change the chapter animation too."*
//
// So the composition moved to `src/chapterStage.tsx`, which owns six silhouettes and every
// timing, and this factory now does what it should always have done: hand the stage the pack's
// HANDWRITING and get out of the way. A pack keeps its plate, its ink and its display voice; it
// stops owning the one shape.
    ...
    return (
      <ChapterStage
        ...
        kit={{
          ink: kit.ink,
          plate: kit.Panel
            ? (children) => React.createElement(kit.Panel as React.FC<Record<string, unknown>>,
                                                {...(kit.panelProps ?? {})}, children)
            : undefined,
        }}
      />
    );
