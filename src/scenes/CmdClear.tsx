...
// CMD_CLEAR — `clear` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: term-buffer — The terminal is drawn as two stacked regions: the visible screen at the bottom and the scrollback stacked above it.
//
...
export const CmdClear: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdClear;
