...
// CMD_GLANCES — `glances` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: gauge-board — Four subsystem panels on one board, each with a health colour driven by its own thresholds.
//
...
export const CmdGlances: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdGlances;
