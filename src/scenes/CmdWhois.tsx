...
// CMD_WHOIS — `whois` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: record-card — The registry record drawn as a card with the expiry date on a real countdown against today's date, so twenty-eight days away reads as urgency rather than a string.
//
...
export const CmdWhois: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdWhois;
