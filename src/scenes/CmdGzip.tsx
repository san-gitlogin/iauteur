...
// CMD_GZIP — `gzip` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: archive-box — The original file is drawn being consumed as the compressed one grows, so the replacement is something you watch happen.
//
...
export const CmdGzip: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdGzip;
