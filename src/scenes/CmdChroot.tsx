...
// CMD_CHROOT — `chroot` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: env-scope — The tree redraws with a new root boundary: everything above the chosen directory is greyed out and cut away, so the process's entire visible universe shrinks on screen.
//
...
export const CmdChroot: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdChroot;
