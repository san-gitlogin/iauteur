...
// CMD_NETHOGS — `nethogs` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: bandwidth-flow — The same bandwidth, but grouped by owning process rather than by address.
//
...
export const CmdNethogs: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdNethogs;
