...
// CMD_PARTED — `parted` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: partition-map — The address field is drawn as a physical row of bits with a hard wall at the 2TB mark.
//
...
export const CmdParted: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdParted;
