...
// CMD_DSTAT — `dstat` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: gauge-board — Three subsystem strips share one time axis, sampled on the same tick.
//
...
export const CmdDstat: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdDstat;
