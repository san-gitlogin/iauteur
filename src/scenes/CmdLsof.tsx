...
// CMD_LSOF — `lsof` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: handle-map — A resource on one side and the processes holding it on the other, joined by real handle lines.
//
...
export const CmdLsof: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdLsof;
