...
// CMD_IOTOP — `iotop` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: proc-live — The live process table, but the sort column is disk throughput rather than CPU.
//
...
export const CmdIotop: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdIotop;
