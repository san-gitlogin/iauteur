...
// MCP_LOOP — the agentic loop as a ring that genuinely cycles, with the exit condition named on the node that ends it.
...
export const McpLoop: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.mcpLoop;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <McpViz kind="agentic-loop" items={cells} ends={ends} accent={accent} />
        </CodeStage>
      </div>
