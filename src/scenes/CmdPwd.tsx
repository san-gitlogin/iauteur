...
// CMD_PWD — `pwd` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: fs-tree — The same tree, but the branch from root down to the current folder lights as one connected trail, and the trail assembles itself into the absolute path string underneath, segment by segment.
//
...
export const CmdPwd: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdPwd;
