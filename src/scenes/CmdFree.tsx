...
// CMD_FREE — `free` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: memory-bar — Memory drawn as one physical bar divided into real segments.
//
...
export const CmdFree: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdFree;
