...
// CMD_MKFS — `mkfs` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: disk-map — The partition's block grid is shown holding real data.
//
...
export const CmdMkfs: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdMkfs;
