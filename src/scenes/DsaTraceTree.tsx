...
// DSA_TRACE_TREE — a tree that descends by indentation, marks a dead end, and visibly walks back up with a backtrack flag.
...
export const DsaTraceTree: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.dsaTree;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <AlgoViz kind="tree" cells={cells} aux={aux} pointers={pointers} accent={accent} />
        </CodeStage>
      </div>
