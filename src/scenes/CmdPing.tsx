...
// CMD_PING — `ping` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: net-path — A packet is drawn actually travelling out to the destination and back, with the round trip time measured as the length of that journey.
//
...
export const CmdPing: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdPing;
