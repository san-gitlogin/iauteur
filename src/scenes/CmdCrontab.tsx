...
// CMD_CRONTAB — `crontab` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: fs-writes — Two routes to the same file drawn side by side: crontab -e passes through a validator that rejects a bad line before installing, while editing the file directly bypasses the validator entirely and the bad line lands unchecked.
//
...
export const CmdCrontab: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdCrontab;
