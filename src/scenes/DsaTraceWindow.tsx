...
// DSA_TRACE_WINDOW — the row with a window frame that physically spans the current sub-array and slides, so growing and shrinking are one continuous motion rather than two states.
...
export const DsaTraceWindow: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.dsaWindow;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <AlgoViz kind="array-window" cells={cells} pointers={pointers} accent={accent} />
        </CodeStage>
      </div>
