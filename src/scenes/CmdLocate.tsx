...
// CMD_LOCATE — `locate` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: index-vs-disk — Two panels: the live disk on one side and a database snapshot stamped 03:00 on the other.
//
...
export const CmdLocate: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdLocate;
