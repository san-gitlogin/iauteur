...
// CMD_HOST — `host` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: dns-resolve — The same resolution drawn three ways at increasing verbosity, so the viewer sees one question answered at three different depths rather than three unrelated tools.
//
...
export const CmdHost: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdHost;
