...
// CMD_NMON — `nmon` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: gauge-board — An empty board that the viewer builds: each keypress slides a new panel in and docks it, so the dashboard visibly assembles itself from nothing rather than appearing complete.
//
...
export const CmdNmon: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdNmon;
