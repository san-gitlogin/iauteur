...
// CMD_DD — `dd` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: disk-map — Two disks drawn as block grids with an arrow between them.
//
...
export const CmdDd: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdDd;
