...
// CMD_DF — `df` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: disk-map — The filesystem drawn twice as two separate physical capacity bars: one for space in gigabytes, one for inodes.
//
...
export const CmdDf: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdDf;
