...
// MCP_FLAGS — the two scary flags as switches, and the grid of features that visibly goes dark when one is thrown.
...
export const McpFlags: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.mcpFlags;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <McpViz kind="flag-matrix" items={cells} ends={ends} accent={accent} />
        </CodeStage>
      </div>
