...
// MCP_URI — resource URIs as routes, where the {id} segment visibly binds to a function parameter and returns real data.
...
export const McpUri: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.mcpUri;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <McpViz kind="uri-router" items={cells} ends={ends} accent={accent} />
        </CodeStage>
      </div>
