...
// MCP_SAMPLING — the direction of the protocol reversing, server to client, with the API bill visibly landing on whichever side holds the key.
...
export const McpSampling: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.mcpSampling;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <McpViz kind="sampling-flip" items={cells} ends={ends} accent={accent} />
        </CodeStage>
      </div>
