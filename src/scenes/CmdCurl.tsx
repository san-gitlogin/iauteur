...
// CMD_CURL — `curl` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: http-exchange — An HTTP request assembled visibly piece by piece — method, headers, body — then sent, with the response drawn as a real status line and headers coming back rather than just a body blob.
//
...
export const CmdCurl: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdCurl;
