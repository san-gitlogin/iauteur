...
// CMD_WATCH — `watch` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: repeat-loop — A real loop: the command runs, its output lands, changed characters are highlighted against the previous run, then a countdown ticks down and it runs again.
//
...
export const CmdWatch: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdWatch;
