...
// MCP_MENTION — the same answer fetched two ways on one clock — resource injection against a tool round trip — so "it is faster" is evidence rather than a claim.
...
export const McpMention: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.mcpMention;
    ...
    <AbsoluteFill>
      ...
      >
        <SoloStage accent={accent} caption={d.caption} vars={vars} premise={d.premise}>
          <McpViz kind="mention-race" items={cells} ends={ends} accent={accent} />
        </SoloStage>
      </div>
