...
// CMD_SS — `ss` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: net-sockets — Sockets drawn in their real states: LISTEN as an open door with nothing behind it, ESTABLISHED as a live pipe with a peer on the far end.
//
...
export const CmdSs: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdSs;
