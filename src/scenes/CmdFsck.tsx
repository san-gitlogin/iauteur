...
// CMD_FSCK — `fsck` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: disk-map — The filesystem's structures are swept in sequence with a moving scan head — inode table, block map, directory tree — and an orphaned inode is found and visibly reattached.
//
...
export const CmdFsck: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdFsck;
