...
// DSA_TRACE_HASH — buckets that receive a key and hold a count, each with a bar that grows as the same key is seen again.
...
export const DsaTraceHash: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.dsaHash;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <AlgoViz kind="hashmap" cells={cells} aux={aux} pointers={pointers} accent={accent} />
        </CodeStage>
      </div>
