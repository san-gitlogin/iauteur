...
// CMD_NETSTAT — `netstat` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: net-sockets — Two paths to the same answer drawn side by side: netstat reading /proc file by file with a visible per-file cost, and ss making one direct kernel call.
//
...
export const CmdNetstat: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdNetstat;
