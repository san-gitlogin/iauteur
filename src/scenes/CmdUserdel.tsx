...
// CMD_USERDEL — `userdel` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: fs-writes — The account line is struck out of /etc/passwd while the home directory card stays fully present on disk, its owner label degrading from a name into a bare numeric UID — the orphaned folder made visible.
//
...
export const CmdUserdel: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdUserdel;
