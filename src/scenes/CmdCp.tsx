...
// CMD_CP — `cp` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: file-bytes — Two file cards side by side.
//
...
export const CmdCp: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdCp;
