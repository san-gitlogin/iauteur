...
// CMD_JOURNALCTL — `journalctl` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: log-filter — The journal is drawn as a queryable store rather than a file, with each flag physically narrowing the visible set — by unit, by boot, by priority — so composition is a shrinking result set you watch.
//
...
export const CmdJournalctl: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdJournalctl;
