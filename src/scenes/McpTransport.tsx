...
// MCP_TRANSPORT — one machine passing notes through stdin and stdout, against two machines talking over HTTP, drawn as the physically different things they are.
...
export const McpTransport: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.mcpTransport;
    ...
    <AbsoluteFill>
      ...
      >
        <SoloStage accent={accent} caption={d.caption} vars={vars} premise={d.premise}>
          <McpViz kind="transport" items={cells} ends={ends} accent={accent} />
        </SoloStage>
      </div>
