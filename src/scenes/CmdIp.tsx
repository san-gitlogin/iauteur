...
// CMD_IP — `ip` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: net-stack — The machine drawn with its real network layers: physical interfaces at the bottom, addresses bound onto them, and a routing table above deciding where traffic exits.
//
...
export const CmdIp: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdIp;
