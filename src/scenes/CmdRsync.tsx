...
// CMD_RSYNC — `rsync` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: sync-diff — Both file sets are drawn side by side and compared before anything moves.
//
...
export const CmdRsync: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdRsync;
