...
// CMD_SCREEN — `screen` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: session-box — The session is drawn as a box that belongs to the server rather than to your connection.
//
...
export const CmdScreen: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdScreen;
