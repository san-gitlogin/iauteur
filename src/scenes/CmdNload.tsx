...
// CMD_NLOAD — `nload` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: bandwidth-flow — Two live meters, in and out, with a scrolling graph under each.
//
...
export const CmdNload: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdNload;
