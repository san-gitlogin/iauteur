...
// CMD_PASSWD — `passwd` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: hash-oneway — A one-way funnel.
//
...
export const CmdPasswd: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdPasswd;
