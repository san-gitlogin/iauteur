...
// CMD_DU — `du` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: disk-map — A treemap where each folder's rectangle is genuinely proportional to its size.
//
...
export const CmdDu: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdDu;
