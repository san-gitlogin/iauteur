...
// CMD_LESS — `less` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: file-viewport — Same viewport, but now it moves freely in both directions, and a search jumps it straight to a match that lights up.
//
...
export const CmdLess: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdLess;
