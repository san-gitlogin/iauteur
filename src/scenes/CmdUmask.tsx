...
// CMD_UMASK — `umask` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: perm-switches — A subtraction drawn on the switch banks: the default 666 lights up, the mask's bits are then physically switched OFF one by one, and the remaining lit switches read out as 644.
//
...
export const CmdUmask: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdUmask;
