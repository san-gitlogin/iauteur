...
// CMD_ENV — `env` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: env-scope — The variable is drawn inside a boundary that belongs to one process only.
//
...
export const CmdEnv: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdEnv;
