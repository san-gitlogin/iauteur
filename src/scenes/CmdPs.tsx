...
// CMD_PS — `ps` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: proc-table — A live process table where each row is a real process card carrying its PID, user and command.
//
...
export const CmdPs: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdPs;
