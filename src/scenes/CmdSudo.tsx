...
// CMD_SUDO — `sudo` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: auth-gate — A gate between the shell and the kernel.
//
...
export const CmdSudo: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdSudo;
