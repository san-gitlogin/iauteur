...
// CMD_NMCLI — `nmcli` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: net-stack — One physical device drawn with several saved profile cards stacked above it.
//
...
export const CmdNmcli: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdNmcli;
