...
// CMD_CHPASSWD — `chpasswd` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: pipe-flow — A stream of user:password pairs flows down a pipe into the command, and each pair is consumed and stamped set as it passes, with a running counter — so bulk is something you watch, not read.
//
...
export const CmdChpasswd: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdChpasswd;
