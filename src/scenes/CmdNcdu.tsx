...
// CMD_NCDU — `ncdu` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: disk-map — The same treemap, but navigable: pressing enter zooms into a folder and redraws, with a breadcrumb trail building along the top so the descent is one continuous movement rather than three separate commands.
//
...
export const CmdNcdu: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdNcdu;
