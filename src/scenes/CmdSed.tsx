...
// CMD_SED — `sed` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: text-transform — Lines flow through a transform box one at a time: the original enters on the left, the matched portion lights inside the box, and the altered line leaves on the right.
//
...
export const CmdSed: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdSed;
