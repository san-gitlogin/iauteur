...
// MCP_SCHEMA — a plain function acquiring its decorator and registering, and the Field(description=...) text arriving as the JSON schema Claude actually reads.
...
export const McpSchema: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.mcpSchema;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <McpViz kind="schema-bind" items={cells} ends={ends} accent={accent} />
        </CodeStage>
      </div>
