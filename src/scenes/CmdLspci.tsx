...
// CMD_LSPCI — `lspci` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: device-ids — Each device is drawn with two independent indicators: present on the bus, and bound to a driver.
//
...
export const CmdLspci: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdLspci;
