...
// CMD_MTR — `mtr` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: net-path — The traceroute map stays on screen and updates continuously, each hop accumulating a real loss percentage over hundreds of probes.
//
...
export const CmdMtr: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdMtr;
