...
// CMD_TMUX — `tmux` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: session-box — The same surviving session, now split into real panes that are visibly separate terminals inside one box, so multiplexing is shown as division of a persistent thing rather than as extra windows.
//
...
export const CmdTmux: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdTmux;
