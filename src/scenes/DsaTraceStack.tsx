...
// DSA_TRACE_STACK — a real stack of plates that grows upward, where only the top is reachable and a popped plate is drawn as a dashed ghost.
...
export const DsaTraceStack: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.dsaStack;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <AlgoViz kind="stack" cells={cells} aux={aux} pointers={pointers} accent={accent} />
        </CodeStage>
      </div>
