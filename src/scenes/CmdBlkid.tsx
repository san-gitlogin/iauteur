...
// CMD_BLKID — `blkid` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: device-ids — Two panels: device names on the left and UUIDs on the right, joined by lines.
//
...
export const CmdBlkid: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdBlkid;
