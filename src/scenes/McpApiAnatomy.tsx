...
// MCP_API_ANATOMY — the Anthropic API call taken apart argument by argument, each one labelled with what it actually controls.
...
export const McpApiAnatomy: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.mcpApi;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <McpViz kind="api-anatomy" items={cells} ends={ends} accent={accent} />
        </CodeStage>
      </div>
