...
import {MemViz, MemItem} from '../memViz';

// MEM_STAGE — one beat of the FluidRAM review.
//
// The scene type is a SHELL. The picture lives in src/memViz.tsx and is chosen by `kind`,
// because the budget a video commits to is the number of distinct PICTURES, not the number
// of registered scene types (LAW 0n corollary). Twenty-five pictures, one plumbing job.
export const MemStage: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.memStage;
  ...
  // mapping is where `parent`/`links`, `out`/`series` and `icon` were each silently dropped
  // on three previous courses (LAW 0n corollary).
  const cells: MemItem[] = (d.cells ?? []).map((c) => ({
    ...
    <AbsoluteFill>
      ...
      >
        <StatePane caption={d.caption} premise={d.premise} vars={vars} accent={accent}>
          <MemViz kind={d.kind ?? ''} items={cells} accent={accent} token={d.token} />
        </StatePane>
      </div>
