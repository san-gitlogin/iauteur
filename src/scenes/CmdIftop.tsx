...
// CMD_IFTOP — `iftop` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: bandwidth-flow — Bandwidth drawn as flows between address pairs, with the width of each flow proportional to its throughput.
//
...
export const CmdIftop: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdIftop;
