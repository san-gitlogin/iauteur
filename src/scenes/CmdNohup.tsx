...
// CMD_NOHUP — `nohup` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: signal-path — The terminal window is closed on screen and a SIGHUP packet visibly propagates down to its child jobs.
//
...
export const CmdNohup: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdNohup;
