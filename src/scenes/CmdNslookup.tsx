...
// CMD_NSLOOKUP — `nslookup` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: dns-resolve — The resolver's cache is drawn explicitly between you and the authoritative server.
//
...
export const CmdNslookup: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdNslookup;
