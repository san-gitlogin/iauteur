...
// CMD_TRACEROUTE — `traceroute` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: net-path — The path is discovered hop by hop: each probe travels one router further before expiring and reporting back, so the map is built outward in front of you.
//
...
export const CmdTraceroute: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdTraceroute;
