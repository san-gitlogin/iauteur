...
// CMD_IOSTAT — `iostat` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: queue-meter — A disk drawn with a real request queue in front of it.
//
...
export const CmdIostat: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdIostat;
