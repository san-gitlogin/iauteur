...
// CMD_WGET — `wget` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: transfer-bar — A download progress bar that genuinely breaks at 61%, then resumes from exactly that point rather than restarting, with the already-downloaded portion visibly retained on disk.
//
...
export const CmdWget: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdWget;
