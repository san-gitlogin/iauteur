...
// CMD_DMESG — `dmesg` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: term-buffer — The kernel ring buffer is drawn as a genuinely circular, fixed-size store: new messages enter at one end and the oldest are pushed out the far side and lost, which is why the buffer can silently forget the message you needed.
//
...
export const CmdDmesg: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdDmesg;
  ...
  const accent = (d.color ?? "orange") as any;
