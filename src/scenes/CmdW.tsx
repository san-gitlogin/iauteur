...
// CMD_W — `w` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: session-list — Each login is drawn as its own terminal card with a TTY label and the command currently running inside it.
//
...
export const CmdW: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdW;
