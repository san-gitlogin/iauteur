...
// CMD_TAR — `tar` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: archive-box — Each letter of czvf is drawn as a separate mechanical action on the pipeline: files gather into one container, the container is squeezed by gzip, names scroll past as they go in, and the output filename is attached at the end.
//
...
export const CmdTar: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdTar;
