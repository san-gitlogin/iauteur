...
// CMD_SCP — `scp` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: key-exchange — A file drawn moving through the same encrypted channel SSH established.
//
...
export const CmdScp: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdScp;
