...
// CMD_FIND — `find` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: fs-walk — The tree is walked visibly, folder by folder, with a moving probe that tests each file and stamps it kept or rejected.
//
...
export const CmdFind: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdFind;
