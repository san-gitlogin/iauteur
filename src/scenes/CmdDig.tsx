...
// CMD_DIG — `dig` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: dns-resolve — The query is drawn travelling to a named resolver and the answer coming back as a real record card with its TTL running down as a visible countdown, so caching becomes a timer rather than a word.
//
...
export const CmdDig: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdDig;
