...
// CMD_BZIP2 — `bzip2` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: archive-box — Two compressors run side by side against a real clock and a real output size, so the trade is a race you watch rather than two numbers you compare.
//
...
export const CmdBzip2: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdBzip2;
