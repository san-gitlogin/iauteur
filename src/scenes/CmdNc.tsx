...
// CMD_NC — `nc` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: port-probe — Three probe outcomes drawn as genuinely different events: an open port completes a handshake, a closed port sends an active refusal back, and a filtered port produces silence with a timeout bar running out.
//
...
export const CmdNc: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdNc;
