...
// CMD_TOP — `top` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: proc-live — The process table is alive: CPU figures move continuously and the rows physically re-sort in front of you, so the busiest process rises to the top as you watch.
//
...
export const CmdTop: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdTop;
