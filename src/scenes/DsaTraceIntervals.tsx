...
// DSA_TRACE_INTERVALS — intervals drawn on a shared timeline at their real start and end, so an overlap is a visible collision and a skipped interval goes dashed.
...
export const DsaTraceIntervals: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.dsaIntervals;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <AlgoViz kind="intervals" cells={cells} pointers={pointers} accent={accent} />
        </CodeStage>
      </div>
