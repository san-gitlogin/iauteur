...
// CMD_MV — `mv` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: fs-inode — The file's data block is drawn once on the right with its inode number printed on it.
//
...
export const CmdMv: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdMv;
