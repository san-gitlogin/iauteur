...
// CMD_HISTORY — `history` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: term-buffer — The history list is drawn as a numbered stack the shell keeps, with each recall form pulling a specific entry back down into the prompt so the substitution is watched rather than described.
//
...
export const CmdHistory: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdHistory;
