...
// CMD_VI — `vi` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: mode-machine — A real mode indicator drawn as a two-state machine with the keyboard beneath it.
//
...
export const CmdVi: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdVi;
