...
// DSA_TRACE_DP — a table whose cells fill one at a time and then stop changing, which is what "solve it once, then only read it" looks like.
...
export const DsaTraceDp: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.dsaDp;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <AlgoViz kind="dp-table" cells={cells} aux={aux} pointers={pointers} accent={accent} />
        </CodeStage>
      </div>
