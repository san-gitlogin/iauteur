...
// CMD_SSH — `ssh` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: key-exchange — Two machines with the key pair drawn as two genuinely different halves.
//
...
export const CmdSsh: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdSsh;
