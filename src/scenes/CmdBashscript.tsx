...
// CMD_BASHSCRIPT — `bash script` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: fs-writes — The file is drawn with its permission bits visible.
//
...
export const CmdBashscript: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdBashscript;
