...
// CMD_TAC — `tac` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: file-content — The same document panel, but the line order visibly inverts: the bottom line lifts to the top and the rest fall in behind it, so the reversal is something you watch happen rather than read about.
//
...
export const CmdTac: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdTac;
