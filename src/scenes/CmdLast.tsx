...
// CMD_LAST — `last` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: session-list — A time-ordered ledger scrolling back through days, each row a login with its timestamp.
//
...
export const CmdLast: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdLast;
