...
// MCP_ELICIT — the current way a server asks the USER for something — a real form with typed fields for form mode, and a browser hand-off for URL mode, where the difference is a security boundary rather than a preference.
...
export const McpElicit: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.mcpElicit;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <McpViz kind="elicit-modes" items={cells} ends={ends} accent={accent} />
        </CodeStage>
      </div>
