...
// CMD_WAIT — `wait` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: timeline-run — Three parallel timelines: two background jobs and the script itself.
//
...
export const CmdWait: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdWait;
