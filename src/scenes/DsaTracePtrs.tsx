...
// DSA_TRACE_PTRS — one beat of a two-pointer trace.
// LEFT: the solution, lit line by line, each with its plain-English note (LAW 0e).
// RIGHT: the sorted row with two flags that travel to their real indices.
// Timing: every element resolves from its own atWord (LAW 0i). No fixed intervals.
export const DsaTracePtrs: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.dsaPtrs;
  ...
  const pointers = (d.pointers ?? []).map((p) => ({
    label: p.label ?? '', at: p.value ?? 0, color: p.color, atWord: p.atWord,
  }));
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <AlgoViz kind="array-ptrs" cells={cells} pointers={pointers} accent={accent} />
        </CodeStage>
      </div>
