...
// CMD_SLEEP — `sleep` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: timeline-run — A script drawn as a vertical timeline with an execution marker travelling down it.
//
...
export const CmdSleep: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdSleep;
