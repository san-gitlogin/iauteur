...
// CMD_MOUNT — `mount` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: mount-tree — The main tree is drawn, then a second disk's tree is grafted onto a chosen folder.
//
...
export const CmdMount: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdMount;
