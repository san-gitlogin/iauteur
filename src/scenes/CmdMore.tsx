...
// CMD_MORE — `more` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: file-viewport — A long document with a viewport frame over it.
//
...
export const CmdMore: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdMore;
