...
// CMD_LSUSB — `lsusb` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: fs-tree — USB is drawn as the physical tree it genuinely is, with a shared bandwidth budget on the hub link that both children visibly draw from — so contention is a picture rather than an explanation.
//
...
export const CmdLsusb: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdLsusb;
