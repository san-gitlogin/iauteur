...
// CMD_ALIAS — `alias` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: env-scope — Two nested scopes drawn as real boundaries: the running shell, and the config file that seeds every future shell.
//
...
export const CmdAlias: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdAlias;
