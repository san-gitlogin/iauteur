...
// CMD_LS — `ls` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: fs-listing — A real directory drawn as file rows with type-coloured names.
//
...
export const CmdLs: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdLs;
