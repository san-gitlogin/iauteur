...
// CMD_SYSTEMCTL — `systemctl` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: service-state — Two independent switches drawn for one service: running now, and enabled at boot.
//
...
export const CmdSystemctl: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdSystemctl;
