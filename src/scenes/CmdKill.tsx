...
// CMD_KILL — `kill` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: signal-path — A signal drawn as a real packet travelling from the shell, through the kernel, into the process.
//
...
export const CmdKill: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdKill;
