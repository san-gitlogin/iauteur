...
// DSA_TRACE_LIST — linked nodes with two runners on them, one moving a step and one moving two, so the closing gap is watched rather than asserted.
...
export const DsaTraceList: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.dsaList;
    ...
    <AbsoluteFill>
      ...
      >
        <CodeStage lines={lines} accent={accent} caption={d.caption} codeTitle={d.codeTitle} vars={vars} premise={d.premise}>
          <AlgoViz kind="linkedlist" cells={cells} aux={aux} pointers={pointers} accent={accent} />
        </CodeStage>
      </div>
