...
// CMD_KILLALL — `killall` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: proc-table — Every process matching the name lights simultaneously across the table, including one clearly owned by another user, and all of them terminate at the same instant — the blast radius is the picture.
//
...
export const CmdKillall: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdKillall;
