...
// CMD_HTOP — `htop` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: proc-live — The same live table, now with a real per-core bar meter across the top, a mouse cursor that selects a row, and an F9 key press that kills the selected process without a PID ever being typed.
//
...
export const CmdHtop: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdHtop;
