...
// CMD_CHMOD — `chmod` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: perm-switches — Nine physical toggle switches in three labelled banks — owner, group, other — each bank showing read, write, execute.
//
...
export const CmdChmod: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdChmod;
