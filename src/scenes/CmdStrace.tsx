...
// CMD_STRACE — `strace` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: syscall-flow — The boundary between a program and the kernel drawn as a real line, with each call crossing it as a labelled request and an answer coming back.
//
...
export const CmdStrace: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdStrace;
