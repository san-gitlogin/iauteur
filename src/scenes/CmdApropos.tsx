...
// CMD_APROPOS — `apropos` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: manual-sections — The search runs across the one-line descriptions of every page rather than the names, so the matched pages are drawn being pulled from several different shelves at once.
//
...
export const CmdApropos: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdApropos;
