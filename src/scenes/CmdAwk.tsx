...
// CMD_AWK — `awk` on the two-up command stage.
// LEFT: a live terminal types each step on the word it is spoken.
// RIGHT: field-split — A single line of output is drawn once, then visibly cut at each run of whitespace into numbered field chips, so $1 $2 $3 are things you watch the line become.
//
...
export const CmdAwk: React.FC<{scene: Scene}> = ({scene}) => {
  const {scale, vertical} = useScale();
  const d = scene.data.cmdAwk;
